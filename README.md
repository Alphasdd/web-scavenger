...
在个人知识库里，LLM 适合做理解、总结和再组织，但前提是原始材料必须先被稳定地保存下来。现实中的输入源却很分散：微信公众号、小红书、普通网页、B 站、抖音、本地视频……每个平台的链接结构、图片防盗链、视频下载、字幕转录方式都不一样。

Web Scavenger 解决的就是这一步：把分散的平台内容统一转成 Obsidian 友好的本地文件。
